...
　　我们用$\{}^{S}W\$来表示在t时刻，传感器坐标系相对于与地球坐标系的姿态。如果初始状态已知的话，则$Q_{W,t}$可由四元数的导数$\dot{Q}_{W,t}$积分得到，如公式 $\ref{4-3}$ 和公式 $\ref{4-4}$ 所示。其中$\hat{Q}_{est,t-1}$表示t-1时刻的姿态估计值，$\{}^{S}W_{t}$为t时刻的陀螺仪测量值，$\Delta t$为采样时间间隔，下标w表示这个四元数是由角速度得到的。
